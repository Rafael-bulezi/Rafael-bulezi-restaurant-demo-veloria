...
export const Hero: React.FC<HeroProps> = ({ onReserve, language }) => {
  ...
  const features = [
    { id: 'fineDining', icon: Utensils, label: t.fineDining.label, desc: t.fineDining.desc },
    { id: 'menu', icon: ScrollText, label: t.menu.label, desc: t.menu.desc },
    { id: 'ocean', icon: Waves, label: t.ocean.label, desc: t.ocean.desc },
    { id: 'heritage', icon: Anchor, label: t.heritage.label, desc: t.heritage.desc },
  ];
  ...
  return (
    <section id="home" ref={heroRef} className="relative h-screen w-full flex flex-col overflow-hidden bg-slate-900">
      <div className="absolute inset-0 z-0">
        {shouldLoadImages && (
          <img src={HERO_IMAGES.default} alt="Default" className="absolute inset-0 w-full h-full object-cover animate-fade-in" />
        )}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 divide-x divide-white/10">
            {features.map((feature, index) => (
              <div 
                ...
                onMouseEnter={() => { setActiveFeature(index); setIsPaused(true); }}
                onMouseLeave={() => setIsPaused(false)}
              >
                ...
                <feature.icon className={`w-7 h-7 mb-3 transition-all duration-500 ${activeFeature === index ? 'text-white scale-110' : 'text-white/70'}`} strokeWidth={1.5} />
